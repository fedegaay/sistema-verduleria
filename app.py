    ...
    for prod, cant in dict_pedidos.items():
        if cant > 0:
            supabase.table("pedidos").insert({
                ...
                "cantidad": round(float(cant), 1), # Redondeo al guardar
                "unidad_medida": dict_unidades[prod],
                "estado": "pendiente"
            }).execute()
    for extra in lista_extras:
        if extra['nombre'] and extra['cantidad'] > 0:
            supabase.table("pedidos").insert({
                ...
                "cantidad": round(float(extra['cantidad']), 1),
                "unidad_medida": extra['unidad'],
                "estado": "pendiente"
            }).execute()
    ...
    with tabs[0]:
        ...
        def render_items():
            for prod in lista_prod_nombres:
                ...
                with c2:
                    # CORRECCIÓN: format="%.1f" muestra solo 1 decimal
            ...
            if info["rol"] == "admin":
                ...
                for i, extra in enumerate(st.session_state.extras):
                    ...
                    with ce2: 
                        st.session_state.extras[i]['cantidad'] = st.number_input(f"C {i}", value=float(extra['cantidad']), min_value=0.0, step=0.5, format="%.1f", key=f"ex_c_{i}", label_visibility="collapsed")
    ...
    if info["rol"] == "admin":
        with tabs[1]:
            ...
            if res.data:
                ...
                # CORRECCIÓN: Formateo de tabla a 1 decimal
        ...
        if hist.data:
            ...
            for rango in df_h['Rango'].unique():
                with st.expander(f"📅 {rango}"):
                    ...
                    # CORRECCIÓN: Visualización de historial a 1 decimal
                    df_render = df_agrupado[['usuarios.nombre_sucursal', 'producto', 'cantidad', 'unidad_medida']].rename(columns={'producto': 'Producto', 'cantidad': 'Total', 'unidad_medida': 'Unidad'})
                    if info["rol"] == "admin":
                        for suc in df_render['usuarios.nombre_sucursal'].unique():
                            st.write(f"📍 **{suc}**")
                            df_suc_final = df_render[df_render['usuarios.nombre_sucursal']==suc][['Producto', 'Total', 'Unidad']]
                            st.dataframe(df_suc_final.style.format({"Total": "{:.1f}"}), hide_index=True)
                    else:
                        st.dataframe(df_render[['Producto', 'Total', 'Unidad']].style.format({"Total": "{:.1f}"}), hide_index=True)